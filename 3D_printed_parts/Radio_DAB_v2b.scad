...
module board() {
 /* difference() {
    ...
    for(pos=pos_scr) {
        ...
        cylinder($fn=100, h=scr_dz*2+b_thickness, r=scr_dia/2);
    }
    translate(pos_b) 
    cube([b_dx, b_dy, b_thickness]);
}
...
module holes() {
    ...
    translate(pos_b)
    translate([5,18+3,-10])
    cylinder($fn=100, h=20, r=3.5/2);

    rotate(0, [1,0,0])
    translate(pos_b)
    translate([11,b_dy-24+3,-10])
    cylinder($fn=100, h=20, r=3.5/2);

    rotate(0, [1,0,0])
    translate(pos_b)
    translate([b_dx-4,b_dy-17+3,-10])
    ...
    translate(pos_b)
    translate([38,7,-10])
    cylinder($fn=100, h=20, r=5/2);

    translate(pos_b) 
    translate([-1,b_dy-16.5,-5]) 
    cube([7+1, 4, 10]);   
}
